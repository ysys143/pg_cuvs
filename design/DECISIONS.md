...
- 알려진 한계:
  ...
  - **완전 자동 극복 방법 (의도적 보류)**: `RegisterSubXactCallback`으로 `SUBXACT_EVENT_ABORT_SUB`를 받아 롤백된 subxact에서 수집된 OID를 `cuvs_pending_drops`에서 제거하는 것이 정공법이다. 단, 각 pending 엔트리에 subxact nesting level을 태깅해야 하고, 현재 commit-only 단순화(ADR-023의 핵심 선택)를 되돌리는 셈이라 복잡도가 올라간다. 이 케이스 자체가 흔치 않아 의도적으로 보류한다(상세 설계는 ADR-060).
