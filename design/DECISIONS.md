...
### 결정 2 — delta_search GUC 문서 정합 (불일치 발견)

`cuvs.delta_search`는 `DefineCustomIntVariable`(정수 0=auto/1=cpu/2=gpu)인데 PLAN·ROADMAP 산문은 `auto|cpu|gpu`(문자열)로 기술 — `SET cuvs.delta_search='auto'`는 `invalid value` ERROR. 본 reconcile에서 **문서를 실제 int 인터페이스(0/1/2)로 정정**(테스트도 0/1/2 사용). 형제 GUC `cuvs.search_mode`는 enum이므로 향후 `delta_search`를 enum(auto|cpu|gpu)으로 전환하면 산문이 그대로 참이 되나, 이는 사용자 대면 인터페이스 변경(기존 `SET ...=1` 파괴)이라 본 스코프 밖 **후속 ergonomics 항목**으로 둔다.
