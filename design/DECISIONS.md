...
**빌드 타임**: `CREATE INDEX USING cagra` 시 raw vector matrix를 `.vectors` sidecar로 직렬화한다 (`<db>_<idx>.vectors`, row-major float32 + `.tids`와 동일한 versioned header). 기존 `.cagra`와 함께 fsync+rename으로 durable하게 저장.

**데몬 로드**: startup 시 `.vectors` sidecar가 존재하면 `CuvsBfIndex`를 별도로 로드하고 `IndexEntry.main_bf_idx`에 보관한다. `.vectors`가 없으면 BF 검색 불가(요청 시 ERROR).

**검색 경로**: IPC search request에 `search_mode` 필드를 추가한다. 데몬은 `search_mode == BF`이면 `main_bf_idx`로 `cuvs_brute_force_search`를 호출하고, `== CAGRA`이면 기존 경로를 유지한다.

**VRAM 영향**: `.vectors` sidecar는 CAGRA 그래프와 별도로 VRAM을 사용한다. 1M×384 float32 = 약 1.5 GB 추가. `pg_stat_gpu_cache`에 `bf_vram_bytes`를 노출한다.
