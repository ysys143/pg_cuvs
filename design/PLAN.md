...
- 검증: regress edge_cases는 write 후 `enable_cuvs=on`에서 stale 인덱스가 새 row를 CPU로 반환함을 단언한다. Integration Sc 10은 100k-row 테이블에서 fresh일 때 planner가 cagra를 고르고, stale 후 EXPLAIN이 Seq Scan으로 바뀌며 정답(id=5, not empty)을 반환함을 단언한다(fix 없으면 empty로 실패하는 진짜 회귀).
