...
2026-05-26 기준 코드베이스는 Phase 1 proof-of-mechanism, Phase 1.5 hardening, 그리고 **제품 로드맵 Phase 2 — Production Ready Single-Node의 코어** 대부분을 통과한 상태다. sidecar, CAGRA build/search, persistence, durable DDL, restart reload, failure injection, large benchmark, crash guard regression에 더해 Phase 2 코어가 들어갔다: `pg_stat_gpu_search`/`pg_stat_gpu_cache` 관측성, `cuvs.k` LIMIT-k, opclass 기반 L2/Cosine/IP metric, k 기반 cost model, write→stale marker(+`.stale` 영속, REINDEX 해소), 빌드 메모리 가드(`cuvs.max_build_mem_mb` 0=auto), VRAM tiered cache(evict-to-fit + reload). 단, 2026-05-26 재검증에서 stale runtime path가 실제 CPU fallback이 아니라 gettuple `return false`에 따른 empty result임이 확인되어 Phase 2.1 hotfix로 재오픈했다. exit 기준 감사는 `docs/phase2-exit-criteria.md`. 잔여(다음 단계): stale fallback 배선 hotfix, build streaming/mmap, delta correction(Phase 3), 자동 backend RSS 기록.
...
- write/delete staleness contract: `aminsert`/`ambulkdelete` mark stale, `.stale` persists it, `REINDEX` clears it. 자동 CPU fallback 배선은 Phase 2.1 hotfix로 재오픈됐다.
...
- write 후 CAGRA는 `.stale` sidecar로 stale 상태를 보존한다. 현재 gettuple STALE branch는 WARNING 후 `return false`라 자동 CPU fallback이 아니라 empty result가 되므로 Phase 2.1에서 stale-aware planner reroute 또는 circuit-breaker 기반 reroute를 먼저 닫아야 한다. stale 상태에서도 GPU path를 유지하는 pending-delta/delta exact search는 Phase 3 필수 기능이다.
...
이 절의 Phase 2는 제품 로드맵 단계다. 목표는 단일 PostgreSQL instance + GPU sidecar 조합을 "데모"가 아니라 운영 가능한 단일 노드 엔진으로 만드는 것이었다. 2026-05-26 기준 **single-node core는 대부분 완료**했지만, stale runtime path의 자동 CPU fallback은 재오픈됐다. 완료 판정과 증거는 `docs/phase2-exit-criteria.md`, feature별 테스트 매핑은 `docs/phase2-test-matrix.md`를 따른다.
...
- **Step 4 — Write/delete/staleness**: `aminsert`와 `ambulkdelete`가 daemon에 stale mark를 보낸다. `.stale` sidecar로 daemon restart 후에도 stale 상태를 보존한다. `REINDEX` 성공 시 fresh로 복구한다. 재검증 결과 stale CAGRA query는 실제 CPU fallback이 아니라 empty result가 되므로 Phase 2.1 hotfix가 필요하다.
...
- **Step 8 — Exit criteria audit**: `docs/phase2-exit-criteria.md`의 10개 기준 중 stale fallback 기준은 `REOPENED`다.
...
- write/delete 후 stale index는 조용히 fresh GPU path로 사용되지 않는다. 그러나 현재 executor path는 CPU fallback 대신 empty result를 반환하므로 Phase 2.1에서 plan-time reroute를 배선해야 한다.
...
목표: Phase 2 문서가 약속한 "stale이면 CPU fallback"을 실제 runtime semantics로 만든다. 현재 STALE IPC status는 gettuple에서 WARNING 후 `return false`를 반환해 PostgreSQL에 "index scan 종료"로 보이며, planner 재라우팅이나 CPU fallback이 발생하지 않는다. 결과적으로 stale query는 정답 CPU path가 아니라 empty result가 될 수 있다.

필수 수정:
- stale 상태를 planner/cost 단계에서 볼 수 있는 process-local 또는 catalog/sidecar cache로 끌어올린다. planner path에서 CUDA/IPC는 여전히 금지한다.
- stale index는 cagra index path cost를 높여 seqscan+sort 또는 pgvector CPU path가 선택되도록 한다. 대안으로 STALE을 circuit breaker에 반영할 수 있으나, stale은 daemon failure가 아니라 per-index data freshness 상태라 별도 stale gate가 더 명확하다.
- gettuple의 STALE branch WARNING 문구는 실제 동작과 일치해야 한다. fallback 배선 전에는 "returning no rows" 또는 ERROR가 맞고, fallback 배선 후에는 planner가 stale path를 고르지 않아야 한다.
- regression/integration test는 `enable_cuvs=off` 수동 fallback이 아니라, write 후 `enable_cuvs=on` 상태에서 stale cagra query가 pgvector/CPU ground truth와 같은 top-k를 반환함을 검증한다.
