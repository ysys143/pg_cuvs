...
| `cuvs.gpu_bruteforce` | enum | `off` | `off`, `auto`, `on` | USERSET | Route a no-index single-table vector top-k query to the transient GPU BF CustomScan (`CuvsTransientBF`). `auto` behaves as `off`: Tier-2 calibration (A100, ADR-069 loop) found transient BF does not beat a parallel CPU seqscan for unfiltered top-k at any tested size, so plan-time auto-routing would be regret-positive — use `on` to opt in explicitly (for always-fresh exactness or CPU-offload under concurrency). `on` enables the path unconditionally for qualifying query shapes (ADR-073) |
