...
| **코스트 모델 v2 — 물리 분해 + 하드웨어-포터블 계수 (ADR-075)** | **설계 확정**(ADR-075). cost = scan+detoast(저장)+move(link)+compute(engine) 물리 분해; 계수 3계층(하드웨어 측정·저장 카탈로그·워크로드 플랜); 하드웨어-포터블 = 데몬 startup probe(`link_bw`/`hbm_bw`/`ipc_rtt`…) → CRC sidecar(정체성 태그) → 플래너 싸게 read, 없으면 보수적 default; detoast-aware base-rel cost로 cagra/flat-vs-seqscan 교차점 교정; recall=exact-우선 단방향(DDL 선택). **구현은 트리거 시**(B 통합메모리 auto / 실측 오보정) | 트리거 |
