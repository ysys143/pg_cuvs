...
| Phase | 내용 | 트랙 |
|-------|------|------|
| **flat B (transient)** | `cuvs.gpu_bruteforce` GUC — 인덱스 없이 vector ORDER BY를 transient GPU-BF CustomScan으로 라우팅(W1: 쓰기 폭주·항상최신·ad-hoc 필터). 설계+적대검증 세션 별도. round-2 FATAL 해결책 확정: Sort 수용 + 실행시 파라미터 바인딩. **flat A1 완료 후 다음 순차 작업** (ADR-073) | 순차 |
| **flat 라우팅 캘리브레이션** | Tier-2 측정 → `cuvs.gpu_bruteforce` off→auto 승격(ADR-069 루프). B 완료 후 | 순차 (B 선결) |
