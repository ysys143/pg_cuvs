...
| **D3 incremental** | ✅ **DONE** | `runner_incremental.py`, 3 scenarios + PK(id) (runs #43–#48). **append** flat 431 / no-index 1573 rows/s; **fifo** (ins+del) flat 345 / no-index 802 ops/s (2.3×); **upsert** 143/192 (pre-PK). **no-index (W1) > flat (W2) across all** (ADR-074 confirmed — flat pays GPU extend + tombstone on top of the heap op). flat's streaming insert/delete/update all functional | follow-ups: recall-drift (window GT recompute), concurrent-query-during-ingest (needs threading) |
