...
| **D3 incremental** | ✅ **DONE** | `runner_incremental.py`, 3 scenarios + PK(id) (runs #43–#49, clean). **append** flat 431 / no-index 1573; **fifo** (ins+del) 345 / 802; **upsert** 417 / 1483 (ops/s) → **no-index (W1) beats flat (W2) by 2.3–3.6× on every write workload** (ADR-074 confirmed — flat pays GPU extend + tombstone on top of the heap op). flat's streaming insert/delete/update all functional | follow-ups: recall-drift (window GT recompute), concurrent-query-during-ingest (needs threading) |
