...
| **D3 incremental** | ✅ **DONE** | `runner_incremental.py`, 3 scenarios (runs #43/#46/#47). **append** flat 431 / no-index 1573 rows/s; **fifo** (ins+del) 80 / 109 ops/s; **upsert** 143 / 192 ops/s — **no-index (W1) > flat (W2) across all** (ADR-074 write-heavy→no-index confirmed). flat's streaming INSERT/DELETE/UPDATE paths all work (no crash). Caveat: table has no PK on `id` → delete/update do a seqscan (id-lookup-bound, inflates absolutes equally) | follow-ups: PK-on-id for clean absolutes; recall-drift (window GT recompute) + concurrent-query-during-ingest |
